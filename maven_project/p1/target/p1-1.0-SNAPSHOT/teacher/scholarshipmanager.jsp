...
<html>
				...
				<tbody>
					<tr>
						<td>标题 <input name="title" value="${title}" class="input-txt"
							...
							name="tno" value="${tno}" /> <input type="hidden"
							name="forwardurl" value="/teacher/scholarshipmanager.jsp" />
	...
	<table id="scholarship" width="100%" border="0" cellspacing="0"
		...
		<tbody>
			<c:if
				test="${listscholarship== null || fn:length(listscholarship) == 0}">
				<tr>
			...
			<%	
							...
							%>
			<tr>
				<td>&nbsp<input id="chk<%=temscholarship.getId()%>"
					class="check" name="ids" type="checkbox"
					value='<%=temscholarship.getId()%>'></td>
				...
				<td>
					<a class="orange-href"
					href="${pageContext.request.contextPath}/teacher/scholarshipdetails.jsp?id=<%=temscholarship.getId()%>">查看</a>
				</td>
			</tr>
			<%}}%>
		</tbody>
