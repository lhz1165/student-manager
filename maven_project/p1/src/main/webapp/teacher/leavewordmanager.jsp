			    ...
			    $("#btnDelete").click(function(){
			        ...
			        $(".check:checked").each(function(index,domEle){
			             ...
			             top.$.dialog.confirm("你确定要注销留言信息?", function(){
				             window.location.href=encodeURI('${pageContext.request.contextPath}/admin/leavewordmanager.do?forwardurl=/teacher/leavewordmanager.jsp&actiontype=delete&id='+id);
				         });
			        });
			    });
				...
				<tbody>
					<tr>
						<td>标题 <input name="title" value="${title}" class="input-txt"
							...
							name="forwardurl" value="/teacher/leavewordmanager.jsp" />
							<div class="ui-button">
			...
			<tr>
				...
				<td>
				 <%if(temleaveword.getStatus()==1){%>
						...
						<a class="orange-href"
							href="${pageContext.request.contextPath}/admin/leavewordmanager.do?actiontype=load&id=<%=temleaveword.getId()%>&tno=<%=temleaveword.getReplyren() %>&forwardurl=/teacher/lwreply.jsp">回复</a>
				<%} %>
				 <%if(temleaveword.getStatus()==2){%>
				                
			      <a class="orange-href"
					href="${pageContext.request.contextPath}/teacher/leaveworddetails.jsp?id=<%=temleaveword.getId()%>"><i
